...
def xgboost_model(df):
    ...
    # Son çekilişi kullanamayız çünkü onu tahmin edeceğiz
    for idx in range(len(df) - 1):
        feature = np.zeros(NUMBERS_RANGE, dtype=int)
        # 1-60 arası sayıları 0-59 indekslere koyuyoruz
        feature[df.loc[idx].values - 1] = 1
        X.append(feature)
        # Sonraki çekilişte çıkan sayılar hedef
    ...
    for i in range(NUMBERS_DRAWN):
        ...
        target = y[:, i]
        # hedefin int olduğundan emin ol
...
def generate_predictions(df, n_preds=1):
    ...
    while len(final_predictions) < n_preds and attempts < n_preds * 100:
        ...
        if len(cond_choice) < NUMBERS_DRAWN:
            remaining = np.setdiff1d(np.arange(1, NUMBERS_RANGE + 1), cond_choice)
            probs = bayes_probs[remaining - 1]
            probs /= probs.sum() if probs.sum() > 0 else 1
            add_nums = np.random.choice(remaining, size=NUMBERS_DRAWN - len(cond_choice), replace=False, p=probs)
        ...
        if len(xgb_choice) < NUMBERS_DRAWN:
            remaining = np.setdiff1d(np.arange(1, NUMBERS_RANGE + 1), xgb_choice)
            probs = bayes_probs[remaining - 1]
            probs /= probs.sum() if probs.sum() > 0 else 1
            add_nums = np.random.choice(remaining, size=NUMBERS_DRAWN - len(xgb_choice), replace=False, p=probs)
        ...
        if len(combined) < NUMBERS_DRAWN:
            remaining = np.setdiff1d(np.arange(1, NUMBERS_RANGE + 1), combined)
            probs = bayes_probs[remaining - 1]
            probs /= probs.sum() if probs.sum() > 0 else 1
            add_nums = np.random.choice(remaining, size=NUMBERS_DRAWN - len(combined), replace=False, p=probs)
...
def main():
    ...
    if uploaded_file:
        ...
        if preds:
            ...
            st.warning("Tahmin üretilemedi, lütfen verinizi kontrol edin veya parametreleri değiştirin.")
